...
mode = st.sidebar.selectbox("Выберите действие:", ["Генерация", "Проверка моего
пароля"])
if mode == "Генерация":
...
if entropy < 40: st.error("Слабый пароль")
elif entropy < 60: st.warning("Средняя надежность")
else: st.success("Высокая надежность")